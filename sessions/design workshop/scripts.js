      ...
      if(show != null){
        ...
        if (s != undefined && e != undefined){
          ...
          data.push($.extend({
            ...
            'has_aired' : has_aired,
            'days_till_airdate' : daysTillAirdata(rec.first_aired),
            'completed': false
          },common));
...
$('main').on('click','.aired:not(.collected,.completed)',function(e){
  e.preventDefault();
...
$('main').on('mouseenter','.aired:not(.collected,.completed)',function(){
...
$('main').on('mouseleave','.aired:not(.collected,.completed)',function(){
  $(this).find('img').stop().transition({
    perspective: '500rem',
    rotateY: '0deg',
    // opacity: 1,
